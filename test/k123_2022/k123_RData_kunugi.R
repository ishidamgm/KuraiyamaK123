...
#csvバージョン
  ...
  for(i in 1:nrows)st_geometry(df)[i]<-st_point(c(t.[i,xc],t.[i,yc]))return(df)
  }
...
#' @examples]
  ...
  k123crown <- list(
   k1=k123crown_RData(d=k1crown,lbl="lbl",sp="sp",a="a",h="h"),
   k2=k123crown_RData(d=k2crown,lbl="lbl",sp="X_sp_i",a="a",h="h"),
   k3=k123crown_RData(d=k3crown,lbl="lbl",sp="sp",a="a",h="h")
  )
...
apg<-read.csv("種名APG.csv",fileEncoding = "cp932")
